<template>
  <div class="footer page__footer">
    <a href="https://4xxi.com/" class="footer__logo">
      <img src="~assets/img/logo-4xxi.svg" alt="logo 4xxi" class="footer__logo_icon footer__logo_icon_4xxi">
    </a>
    <div class="footer__cast cast">
      <a class="cast__app">About CaesarApp</a>.
      <div class="cast__com">
        <span class="cast__team">Enjoy! Build by <a class="cast__com cast__com_link" href="https://4xxi.com/">4xxi</a> team.</span> From Saint Petersburg with <div class="heart"></div>
      </div>
    </div>
  </div>
</template>
